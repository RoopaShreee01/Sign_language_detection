...
GREEN_LOW_LIMIT=np.array([30, 0, 0])
GREEN_HIGH_LIMIT=np.array([80, 255, 255])
PINK_LOW_LIMIT=np.array([140,0,0])
PINK_HIGH_LIMIT=np.array([170,255,255])
YELLOW_LOW_LIMIT=np.array([10, 0, 0])
YELLOW_HIGH_LIMIT=np.array([40, 255, 255])
...
def getGlovesImage(image, color):
  ...
  if color == 'blue' :
    ...
    low_limit = YELLOW_LOW_LIMIT
    high_limit = YELLOW_HIGH_LIMIT
...
while True:
    ...
    masked_image = getGlovesImage(roi, 'green')
